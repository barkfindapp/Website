...
import { APP_STORE_URL } from "../data/launch";
...
const MYLO_SHOTS = [
  { image: "/media/mylo-1.png", caption: "Tell Mylo what you're after" },
  { image: "/media/mylo-2.png", caption: "Mylo sniffs out the best matches" },
  { image: "/media/mylo-3.png", caption: "Straight to a real, rated place" },
];
...
const FEATURES = [
  ...
  },
  {
    ...
    description: "Keep the places that worked in one list, so your go-to cafes, parks and pubs are one tap away next time.",
    icon: (
      <svg className="w-6 h-6" fill="none" stroke="currentColor" strokeWidth={1.75} viewBox="0 0 24 24">
  ...
  {
    title: "Treats for reviewing",
    description: "Once you're using BarkFind, honest reviews earn treats: a little back on your subscription for helping other owners find their way.",
    icon: (
      <svg className="w-6 h-6" fill="none" stroke="currentColor" strokeWidth={1.75} viewBox="0 0 24 24">
      ...
      <div className="max-w-6xl mx-auto px-6">
        <div className="flex flex-col lg:flex-row items-center gap-12 lg:gap-16">
          ...
          <div className="flex-1 w-full max-w-md mx-auto lg:mx-0">
            ...
            <p className="text-center text-xs text-[#585858]">Monthly: a discount off your next bill · Annual: time added to your subscription · 50 points: a free year on either plan</p>
          </div>
        </div>
      </div>
    </section>
...
// Prices are the single source of truth. The savings badge and "months free"
// are derived from them, never hardcoded, so they can't silently go stale if a
// price changes. Months are rounded DOWN so the claim is never overstated.
const MONTHLY_PRICE = 5.99;
const ANNUAL_PRICE = 39.99;
const YEAR_AT_MONTHLY = MONTHLY_PRICE * 12;
...
const PLANS = [
  ...
  {
    name: "Annual",
    price: `£${ANNUAL_PRICE.toFixed(2)}`,
    period: "per year",
    description: `Best value: just £${(ANNUAL_PRICE / 12).toFixed(2)}/month.`,
    highlight: true,
    badge: `Save ${SAVE_PCT}%`,
                ...
                {plan.features.map((f) => (
                  <li key={f.text} className="flex items-start gap-2.5 text-sm">
                    ...
                    {"href" in f && f.href ? (
                      <a
                        href={f.href}
                        className={`underline decoration-dotted underline-offset-2 hover:opacity-80 ${f.strong ? "font-bold" : ""} ${plan.highlight ? "text-white/90" : "text-[#1a1a1a]"}`}
                      >
                        {f.text}
                      </a>
                    ) : (
...
const FAQ_ITEMS = [
  ...
  { q: "How do the review rewards work?", a: "Once you're using BarkFind, leaving honest reviews earns treats: a little back on your subscription for helping other owners find their way. What you get depends on whether you're on the monthly or annual plan. The full rules, and how claiming works, are on the Treats page." },
  { q: "Can I list my business on BarkFind?", a: "Yes. Business owners can claim their listing for free, and more business features are on the roadmap." },
    ...
    <footer className="bg-[#1a1a1a] text-white/50 py-14">
      <div className="max-w-6xl mx-auto px-6 flex flex-col md:flex-row items-center justify-between gap-8">
        ...
        <div className="flex items-center gap-4">
          <a
            href="https://www.instagram.com/barkfind"
            target="_blank"
          ...
          <a
            href="https://www.tiktok.com/@barkfindapp"
            target="_blank"
            rel="noopener noreferrer"
            className="hover:text-white transition-colors"
            aria-label="TikTok"

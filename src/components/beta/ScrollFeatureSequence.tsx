...
// Each step supplies its own still, or null while its screenshot is still pending.
const STEPS = [
  ...
  { key: "profile", still: null, eyebrow: "Somewhere that suits your dog", title: "Matched to your dog, not just any dog", body: "Tell BarkFind your dog's temperament and indoor comfort, and it leans towards places that actually suit them." },
...
function StepStill({ step }: { step: (typeof STEPS)[number] }) {
  if (!step.still) return <AssetPlaceholder label={`Screenshot: ${step.key}`} rounded="rounded-none" />;
